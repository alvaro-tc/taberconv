...
import { fetchCurrentUser } from '../api/ApiCollection';
import {
  LineChart,
  Line,
  XAxis,
                ...
                <div className="flex flex-col items-start gap-1">
                  {isLoading ? (
                    ...
                    <h3 className="font-semibold text-xl xl:text-3xl dark:text-white">
                      {data.user_details.name || 'N/A'} {data.user_details.lastname || 'N/A'}
                    </h3>
                  ) : (
                    <div className="w-[200px] h-[36px] skeleton dark:bg-neutral"></div>
                  )}
                ...
                <div className="w-full grid grid-cols-3 xl:flex gap-5 xl:gap-8">
                  ...
                  <div className="col-span-2 flex flex-col items-start gap-3 xl:gap-5">
                    <span className="font-semibold">
                      {data.user_details.name || 'N/A'}
                    </span>
                    <span className="font-semibold">
                      {data.user_details.lastname || 'N/A'}
                    </span>
                    <span className="font-semibold">
                      {data.user_details.email}
                    </span>
                    <span className="font-semibold">
                      {data.user_details.cellphone || 'N/A'}
                    </span>
                    <span className="font-semibold">
                      {data.user_details.roles.join(', ')}
                    </span>
                  </div>
                </div>
              ) : (
            ...
            <ul>
              <li>
                <div className="ml-[1px] relative p-4 bg-base-200 dark:bg-neutral dark:text-neutral-50 min-w-[85vw] xl:min-w-[480px] flex flex-col items-start gap-3">
                  <span>
                    {data.user_details.name || 'N/A'} {data.user_details.lastname || 'N/A'} se registró el día {new Date().toLocaleDateString()}
                  </span>
                  <span className="text-xs"> ({formatDistanceToNow(new Date())} ago)</span>
                </div>
              </li>
            </ul>
          )}
